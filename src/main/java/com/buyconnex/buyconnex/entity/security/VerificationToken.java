...
@Entity
@Table(name = "EXPIRE_VERIF_TOKEN")
@NoArgsConstructor
@Getter
@Setter
public class VerificationToken {

    private static final int EXPIRATION_RESET_PASSWORD_TOKEN = 60*2;
    private String token;


    @Id
    @SequenceGenerator(name = "EXPIRE_VERIF_TOKEN_SEQ_ID", sequenceName = "SEQ_OID", allocationSize = 1)
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "EXPIRE_VERIF_TOKEN_SEQ_ID")
    @Column(name = "ID")
    private Long id;

    @JoinColumn(nullable = false, name = "ID_USER", foreignKey = @ForeignKey(name = "FK_VERIF_TOKEN_USER"))
    ...
    public VerificationToken(Users users, String token, Date dateExpire) {
        this.users = users;
        this.token = token;
